        ...
        def traverse(root):
            ...
            if root.l_bound == root.r_bound:
                if index == root.l_bound:
                    root.sum_ = val
                return root.sum_
            l_child_sum, r_child_sum = root.l_child.sum_, root.r_child.sum_
            if root.l_child.l_bound <= index <= root.l_child.r_bound:
    ...
    def query(self, L: int, R: int) -> int:
        ...
        def getRangeSum(root):
            ...
            if root.l_child == root.r_child:
                ...
                return 0
            l_sum, r_sum = 0, 0
            if root.l_child.l_bound <= L <= R <= root.l_child.r_bound:
                l_sum = getRangeSum(root.l_child)
            elif root.r_child.l_bound <= L <= R <= root.r_child.r_bound:
