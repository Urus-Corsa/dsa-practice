...
class Solution:
    def leadsToDestination(self, n: int, edges: List[List[int]], source: int, destination: int) -> bool:
        ...
        def dfs(node, seen, successfully_reached_from_no_cycle):
            if node in seen:
                return False
            if node in successfully_reached_from_no_cycle:
                return True
            if node != destination and not adj_list[node]:
                return False
            seen.add(node)
            for neigh in adj_list[node]:
                if not dfs(neigh, seen, successfully_reached_from_no_cycle):
                    return False
            seen.remove(node)
            successfully_reached_from_no_cycle.add(node)
            return True
