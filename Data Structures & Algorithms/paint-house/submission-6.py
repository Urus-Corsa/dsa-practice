            ...
            for color_choice in range(2, -1, -1):
                ...
                for prev_house_color_index in range(2,-1,-1):
                    if prev_house_color_index == color_choice:
                        continue
                    min_cost_from_coloring_prev_with_prev_house_color_index = min(min_cost_from_coloring_prev_with_prev_house_color_index, dp[house_index+1][prev_house_color_index])
                dp[house_index][color_choice] = min_cost_from_coloring_prev_with_prev_house_color_index + costs[house_index][color_choice]
        return min(dp[0])
